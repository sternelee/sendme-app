...
const getClientJWT = () =>
  invoke("plugin:clerk|get_client_authorization_header");
...
const urlForRequestInput = (input) =>
  typeof input === "string"
    ? new URL(input)
...
const shouldRunTauriFetch = (input, init) => {
  ...
  if (initHeaders) {
    if (initHeaders instanceof Headers) {
      if (initHeaders.has("x-no-origin")) return false;
      return initHeaders.has("x-tauri-fetch");
    } else if (Array.isArray(initHeaders)) {
      if (initHeaders.some((h) => h[0] === "x-no-origin")) return false;
      return initHeaders.some((h) => h[0] === "x-tauri-fetch");
    } else {
      if (initHeaders["x-no-origin"]) return false;
      return !!initHeaders["x-tauri-fetch"];
    }
  }
  if (input instanceof Request) {
    if (input.headers.has("x-no-origin")) return false;
...
const proxyClerkRequest = async (input, init) => {
  ...
  if (initHeaders instanceof Headers) {
    ...
    headerTuples.push(...initHeaders);
  } else if (initHeaders) {
    for (const [k, v] of Object.entries(initHeaders)) headerTuples.push([k, v]);
  }
  let body = undefined;
...
const clerkClientToClientJSON = (client) => ({
  ...
  cookie_expires_at: client.cookieExpiresAt
    ? client.cookieExpiresAt.getTime() / 1e3
    : null,
...
const clerkEnterpriseAccountToEnterpriseAccountJSON = (enterpriseAccount) => ({
  ...
  last_authenticated_at: enterpriseAccount.lastAuthenticatedAt
    ? enterpriseAccount.lastAuthenticatedAt.getTime() / 1e3
    : null,
});
...
const clerkUserToUserJSON = (user) => ({
  ...
  legal_accepted_at: user.legalAcceptedAt
    ? user.legalAcceptedAt.getTime() / 1e3
    : null,
...
const initClerk = async (initArgs, intLogger) => {
  ...
  if (isNewInstance) {
    ...
    __internalClerk.addListener(
      ({ client: client$1, session, user, organization }) => {
        emitClerkAuthEvent({
  ...
  try {
    ...
    logger.warn(
      { error: e },
      "Plugin:clerk: Failed to load Clerk UI components from CDN. Pre-built UI components (SignIn, UserButton, etc.) will not be available.",
    );
  }
