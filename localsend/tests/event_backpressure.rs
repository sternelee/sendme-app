...
///
/// Requires a loopback range where every 127.0.0.x address routes (Linux).
/// macOS only routes 127.0.0.1 by default, so the scan finds a single device.
#[cfg_attr(target_os = "macos", ignore = "macOS only routes 127.0.0.1 by default")]
#[tokio::test(flavor = "multi_thread")]
